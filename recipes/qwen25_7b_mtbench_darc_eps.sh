...
"$PYTHON" -m darc.perturb \
  ...
  --out "$RUN_DIR/perturbed.jsonl" \
  --rewriter-model "$REWRITER_MODEL" \
  --cache-dir "$CACHE_DIR" \
  --download \
  --naug "${NAUG:-8}" \
  --mode "${PERTURB_MODE:-style}" \
...
"$PYTHON" -m darc.reward \
  --perturbed "$RUN_DIR/perturbed.jsonl" \
  --out "$RUN_DIR/scored_context.jsonl" \
...
"$PYTHON" -m darc.select \
  --scored "$RUN_DIR/scored_context.jsonl" \
...
"$PYTHON" -m darc.export_results \
  --selected "$RUN_DIR/selected_context_darc_eps_cleanbudget_target_met.jsonl" \
  --method darc_eps \
  --model-id "${MODEL_ID:-qwen25_7b_darc_eps}" \
  --out "$RUN_DIR/mtbench_qwen25_7b_darc_eps_answers.jsonl"
